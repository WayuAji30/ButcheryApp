...
<div class="w-full h-screen bg-[#f3f3f3] overflow-x-hidden sm:overflow-x-auto border-t flex flex-col">
    <div class="lg:mt-32 md:mt-24 sm:mt-20 bg-white rounded-2xl w-[97%] sm:w-[160%] ml-4 sm:mx-3 shadow-md">
        <p class="mt-7">
            <a class="text-[#D10B05] lg:text-[20px] md:text-[18px] pb-4 lg:px-11 md:px-5 px-3 border-b-4 border-[#D10B05] font-medium cursor-default">Daftar
                Produk</a>
        </p>
            ...
            <div class="flex gap-5">
                <form action="/daftarProduk/{{$supplier->_id}}" method="GET">
                    <div class="relative flex items-center">
                        <input type="text" name="cari_daftarproduk" id="cari_daftarproduk" placeholder="Cari Nama Produk" class="border-solid border-2 border-slate-300 rounded-md lg:w-64 md:w-48 py-2 sm:py-1 pl-3 pr-9 focus:outline-[#D10B05]" />
                        <button class="absolute right-3">
                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 24 24" class="md:w-5">
                ...
                <select name="kategori" id="kategori" class="px-3 py-2 sm:py-1 border-solid border-2 border-[#CCCCCC] text-[#999] rounded-md lg:w-36 md:w-24 w-[105px] focus:outline-[#D10B05]">
                    <option value="">Kategori</option>
                    @foreach ($kategori as $k)
                    <option value="{{$k['nama_kategori']['slug']}}">{{$k['nama_kategori']['nama']}}</option>
                    @endforeach
                </select>
                <select name="filter" id="filter" class="px-3 py-2 sm:py-1 border-solid border-2 border-[#CCCCCC] text-[#999] rounded-md lg:w-36 md:w-24 w-[105px] focus:outline-[#D10B05]">
        ...
        <!-- TABLE -->
        <table id="produk-table" class="table-auto w-full mt-5 mb-5 md:text-sm" data-aos="fade-right" data-aos-duration="400" data-aos-easing="ease-in-out">
            <thead>
            ...
            <tbody>
                ...
                <tr class="border-b-2 border-slate-200" data-kategori="{{(isset($dp['id_kategori']) ? $dp['id_kategori'] : '')}}" data-filter="ASC" data-filter2="DESC">
                    <td>
                        <img src="{{asset('storage/img_uploaded/'.(isset($dp['foto']['foto1']) ? $dp['foto']['foto1'] : ''))}}" alt="" class="lg:w-[84px] md:w-14 w-12 rounded-lg ml-5 py-5" />
                    </td>
                    ...
                    <td class="text-center sm:flex sm:flex-col sm:text-sm sm:gap-1">
                        ...
                        <label for="my_modal_6" class=" bg-[#D10B05] py-2 lg:px-8 md:px-3 px-3 sm:mr-2 rounded-md font-semibold text-white hover:bg-[#9F0804] hover:border-[#9F0804] transition-all duration-200 ease-linear btn-hapus" data-id_produk="{{(isset($dp->_id) ? $dp->_id : '')}}">Hapus</label>
                            ...
                            <div class="modal-box">
                                <div class="flex justify-center items-center">
                                    <div class="text-center">
                                        <svg class="mt-3 mx-auto my-auto" xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 40 40">
                                            ...
                                            <path fill="#d10b05" d="M19.029 15.549v8.701a1.136 1.136 0 0 0 2.27 0v-8.701a1.14 1.14 0 0 0-1.135-1.139c-.623 0-1.135.513-1.135 1.139zm1.136 11.35c-.624 0-1.135.506-1.135 1.132v.761a1.14 1.14 0 0 0 1.135 1.132c.626 0 1.135-.513 1.135-1.132v-.761c0-.626-.51-1.132-1.135-1.132zm0 0" />
                                        </svg>
                                        <p class="font-semibold text-[20px] mt-2">Apakah anda yakin
                                            ingin menghapus data ini?</p>
                                        <div class="flex justify-center gap-10">
                                            <div class="modal-action">
                                                <label for="my_modal_6" class="border-2 border-[#d10b05] rounded-md px-8 py-2 font-semibold text-[#d10b05] hover:bg-[#D10B05] hover:text-white transition-all duration-200 ease-linear cursor-pointer">Batal</label>
                                            </div>
